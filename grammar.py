...
class Grammar:
    ...
    def first_of_sequence(self, symbols: tuple[str, ...]) -> set[str]:
        ...
        for symbol in symbols:
            # Se o símbolo for terminal, adicione-o e não é necessário continuar
    ...
    def build_first(self) -> None:
        ...
        # Inicializar FIRST para cada não terminal
        for nonterminal in self.nonterminals:
        ...
        while (changed):
            ...
            for nonterminal in self.nonterminals:
                for production in self._by_lhs[nonterminal]:
                    ...
                    old_size = len(self.first[nonterminal])
                    
                    # Se produz EPSILON, adicione EPSILON ao conjunto FIRST do não terminal
                    if not production.rhs:
                        self.first[nonterminal].add(EPSILON)
                    # Se não produz EPSILON, adicione os símbolos FIRST da sequência de produção
                    else:
                        self.first[nonterminal].update(self.first_of_sequence(production.rhs))
                        
                    # Para o loop se não houveram mudanças
                    if len(self.first[nonterminal]) > old_size:
                        changed = True
        return
    ...
    def build_start(self) -> None:
        ...
        # Inicializar START para cada produção
        for production in self.productions:
            self.start[production] = set()
            
        for nonterminal in self.nonterminals:
            for production in self._by_lhs[nonterminal]:
                # Calcular FIRST da produção
                first_set = self.first_of_sequence(production.rhs)
                
                # Se a produção puder produzir EPSILON, START é o FIRST da produção menos EPSILON, mais FOLLOW do não terminal
                if EPSILON in first_set or not production.rhs:
                    self.start[production].update(first_set - {EPSILON})
                    self.start[production].update(self.follow[nonterminal])
                # Senão, START é o FIRST da produção
                else:
                    self.start[production].update(first_set)
    ...
    def eliminate_direct_left_recursion(self, nonterminal: str) -> bool:
        ...
        #print(f"Eliminando recursão direta para o não terminal {nonterminal}...")
        
        # Separar produções dependentes e independentes
        alphas = []
        betas = []
        for production in self._by_lhs[nonterminal]:
            if production.rhs and production.rhs[0] == nonterminal: # Produção tem recursão direta
                alphas.append(production.rhs[1:])
            else: # Produção não tem recursão direta
                betas.append(production.rhs)
        
        # Verificar se há recursão direta à esquerda
        if not alphas: return False
        
        # Criar um novo não terminal
        helper = self._fresh_nonterminal(nonterminal)
        self._insert_nonterminal_after(nonterminal, helper)
        
        # Alterar a gramática
        self._replace_productions(helper, [alpha + (helper,) for alpha in alphas] + [()])
        self._replace_productions(nonterminal, [beta + (helper,) for beta in betas])
